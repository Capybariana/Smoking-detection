...
async def process_queue(context: ContextTypes.DEFAULT_TYPE):
    ...
    processing_message = await context.bot.send_message(chat_id=chat_id,
                                                        text="Настала ваша очередь. Видео принято в обработку. Пожалуйста, ожидайте.")
    ...
    try:
        ...
        async with Session() as session:
            video_entry = Video(file_path=original_filename,
                                status="processing")
        ...
        processed_output_path = await asyncio.to_thread(run_full_pipeline,
                                                        original_filename)
